    ...
    return (
        <div ref={ref} className="p-10 bg-white text-black font-sans print:p-6" style={{width: "100%"}}>
            ...
            <div className="flex justify-between items-start border-black pb-6">
                <div className="flex gap-4 items-center">
                    ...
                    <div>
                        ...
                        <p className="text-[11px] font-bold uppercase tracking-[0.2em] mt-2">Liquidación de Fletes y
                            Gastos</p>
                    </div>
                </div>
            ...
            <div className="grid grid-cols-3 gap-6 items-start mb-6">
                ...
                {/* COLUMNA IZQUIERDA: TABLA ULTRA COMPACTA (OCUPA 2 DE 3 ESPACIOS) */}
                        ...
                        {viajeData.vehiculos?.map((v, i) => (
                            <tr key={i} className="border-b border-gray-400 text-[10px] leading-none">
                                <td className="p-1 border-r border-gray-300">
                                    ...
                                    <span
                                        className="text-[8px] font-bold text-gray-500 ml-2 uppercase truncate max-w-[100px]">
                                {v.marca} {v.modelo}
                            </span>
                                </td>
                ...
                {/* COLUMNA DERECHA: RESUMEN Y TOTALES (OCUPA 1 DE 3 ESPACIOS) */}
                <div className="col-span-1 flex flex-col gap-4">
                    {/* RESUMEN DETALLADO */}
                    <div className="border-4 border-black p-3 rounded-xl shadow-lg bg-gray-50">
                        <h4 className="text-[10px] font-black uppercase mb-3 border-b border-black pb-1">Resumen de
                            Liquidación</h4>
                        <div className="space-y-1.5">
                            <div className="flex justify-between text-[10px] font-bold uppercase">
                                <span>Fletes:</span>
                                <span>${viajeData.resumenFinanciero?.totalFletes?.toLocaleString()}</span>
                            </div>
                            <div className="flex justify-between text-[10px] font-bold uppercase">
                                <span>Storages:</span>
                                <span>${viajeData.resumenFinanciero?.totalStorage?.toLocaleString()}</span>
                            </div>
                            <div className="flex justify-between text-[10px] font-bold uppercase">
                                <span>S. Pesos:</span>
                                <span>${viajeData.resumenFinanciero?.totalSobrepeso?.toLocaleString()}</span>
                            </div>
                            <div className="flex justify-between text-[10px] font-bold uppercase">
                                <span>G. Extras:</span>
                                <span>${viajeData.resumenFinanciero?.totalGastosExtra?.toLocaleString()}</span>
                            </div>

                            <div className="border-t-2 border-black border-dashed mt-2 pt-2">
                                <div
                                    className="flex justify-between text-lg font-black uppercase text-black leading-none">
                                    <span>Total:</span>
                                    <span>${viajeData.resumenFinanciero?.granTotal?.toLocaleString()}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    {/* CLÁUSULA COMPACTA */}
                    <div
                        className="text-[8px] text-justify italic leading-tight text-gray-500 border border-gray-200 p-2 rounded">
                        <p className="font-bold text-black mb-1 uppercase tracking-tighter">Cláusula de Conformidad:</p>
                        Recibí la cantidad mencionada a mi entera satisfacción por concepto de liquidación total del
                        viaje {viajeData.numViaje}. Al firmar, otorgo finiquito total y libero a la empresa de
            ...
            <div className="mt-24 grid grid-cols-2 gap-32 px-12">
                <div className="text-center">
                    <div className="border-t-2 border-black pt-2">
                        ...
                        <p className="text-[10px] text-gray-400 uppercase font-bold tracking-widest mt-1 italic">Nombre
                            y Firma del Chofer</p>
                    </div>
                </div>
                <div className="text-center">
                    <div className="border-t-2 border-black pt-2">
                        <p className="text-xs font-black uppercase italic">{viajeData.pagadoPor?.nombre || "ADMINISTRACIÓN"}</p>
                        <p className="text-[10px] text-gray-400 uppercase font-bold tracking-widest mt-1 italic">Autorizado
                            por Caja</p>
                    </div>
                </div>
            </div>
            ...
            <div className="mt-4 text-center">
                <p className="text-[9px] font-black text-gray-300 uppercase italic tracking-[0.3em]">
                    Documento de Control No Reutilizable
                </p>
            </div>
        </div>
    );
