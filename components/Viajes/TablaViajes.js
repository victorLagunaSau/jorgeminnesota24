...
const TablaViajes = ({user}) => {
    ...
    return (
        <div className="bg-white-500 rounded-xl shadow-md border border-gray-100 font-sans overflow-hidden text-black">
            ...
            {modal.show && (
                <div
                    className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 text-black">
                    <div
                        className="bg-white-500 rounded-xl p-8 max-w-sm w-full border-t-8 border-red-600 shadow-2xl font-sans">
                        ...
                        <div className="flex justify-end gap-3 mt-8">
                            ...
                            {modal.accion && <button onClick={modal.accion}
                                                     className="btn btn-sm btn-error text-white-500 font-black uppercase text-[10px]">Continuar</button>}
                        </div>
                    </div>
                </div>
            )}
            ...
            <div className="p-4 bg-gray-50 border-b flex flex-wrap justify-between items-center gap-4">
                ...
                <div className="flex items-center gap-2 bg-white-500 px-3 py-1 rounded-md border shadow-sm">
                    ...
                    <input type="text" placeholder="Filtrar viaje..."
                           className="bg-transparent outline-none text-[12px] w-64 font-bold uppercase"
            ...
            <div className="overflow-x-auto">
                <table className="table table-compact w-full border-separate border-spacing-0">
                    ...
                    <tr className="text-[9px] uppercase text-gray-400 bg-white-500 sticky top-0 z-20 shadow-sm">
                        ...
                        <th className="p-3 border-r border-b text-center">Titulo</th>
                    ...
                    {filtrados.map((viaje, vIndex) => (
                        viaje.vehiculos.map((v, idx) => {
                            ...
                            return (
                                <tr key={`${viaje.id}-${idx}`}
                                    className={`hover:bg-blue-50/20 transition-colors ${vIndex % 2 === 0 ? "bg-white-500" : "bg-gray-50"}`}>
                                    {idx === 0 && (
                                        <td rowSpan={viaje.vehiculos.length}
                                            className="border-r border-b p-3 align-top bg-gray-50/20 w-44">
                                            <div className="flex flex-col gap-1">
                                                <span
                                                    className="text-sm font-black text-red-600">#{viaje.numViaje}</span>
                                                <span
                                                    className="text-[11px] font-black uppercase text-gray-800">{viaje.chofer?.nombre}</span>
                                                <span
                                                    className={`text-[8px] font-bold uppercase px-2 py-0.5 rounded-full mt-2 w-fit ${viaje.estatus === 'PENDIENTE' ? 'bg-yellow-100 text-yellow-600' : 'bg-blue-100 text-blue-600'}`}>
                                                        {viaje.estatus}
                                                    </span>
                                            </div>
                                        </td>
                                    ...
                                    <td className="border-r border-b p-1 text-center">
                                        {user.admin ? (
                                            <select
                                                ...
                                                className="select select-bordered select-xs w-full font-bold text-[10px]"
                                            >
                                                <option value="NO">NO</option>
                                                <option value="SI">SI</option>
                                            </select>
                                        ) : (
                                            <span className="text-[10px] font-bold">{v.titulo}</span>
                                        )}
                                    </td>

                                    {idx === 0 && (
                                        <td rowSpan={viaje.vehiculos.length}
                                            className="border-b p-3 bg-blue-50/10 align-middle text-center w-40">
                                                        ...
                                                        {viaje.estatus === "EN VERIFICACION" && (
                                                            <button onClick={() => setModal({
                                                                ...
                                                                accion: () => cambiarEstatus(viaje.id, "VERIFICADO")
                                                                    ...
                                                                    className="btn btn-sm btn-info text-white-500 font-black text-[9px] uppercase leading-tight py-2 h-12">
                                                                Confirmar<br/>Verificación
                                                            </button>
                                                        )}
                                                        {viaje.estatus === "VERIFICADO" && (
                                                            <button onClick={() => setViajeALiquidar(viaje)}
                                                                    className="btn btn-sm btn-success text-white-500 font-black text-[10px] h-12 uppercase italic">
                                                                Pagar Viaje
                                                            </button>
                                                        )}
                                                        <ReactToPrint
                                            ...
                                            {/* BOTÓN DINÁMICO PARA REGRESAR ESTATUS (CAMBIAR DATOS) */}
                                            {user.admin && viaje.estatus !== "PENDIENTE" && (
                                                <button
                                                    ...
                                                    className="btn btn-xs btn-ghost text-red-400 font-bold uppercase underline text-[8px] mt-1"
                                                >
                                                    Editar
                                                </button>
                                            )}
