    ...
    return (
        <div className="fixed inset-0 z-[110] flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
            ...
            <div style={{ display: "none" }}>
                <ReciboPago
                    ref={reciboRef}
                    viajeData={{ ...viaje, folioPago: folioGenerado, fechaPago: new Date(), empresaLiquidada: empresaSeleccionada, pagadoPor: user }}
                />
            </div>

            {showConfirm && (
                <div className="fixed inset-0 z-[120] flex items-center justify-center bg-black/40 p-4 font-sans text-black">
                    <div className="bg-white-500 rounded-xl p-8 max-w-sm w-full shadow-2xl border-2 border-t-8 border-red-600 animate-in zoom-in duration-200">
                        <h3 className="text-xl font-black uppercase tracking-tighter italic">Confirmar Liquidación</h3>
                        <div className="my-4 p-4 bg-gray-50 rounded-lg border border-dashed border-red-200 leading-tight">
                            ...
                            <p className="text-[10px] font-black text-gray-400 uppercase italic">Total de la Operación:</p>
                            <p className="text-3xl font-black text-gray-800">${granTotalReal.toLocaleString()}</p>
                        </div>
                        <div className="flex justify-end gap-3 mt-6">
                            <button onClick={() => setShowConfirm(false)} className="btn btn-sm btn-ghost font-black uppercase text-[10px]">Cerrar</button>
                            <button onClick={ejecutarPago} className="btn btn-sm btn-error text-white-500 font-black uppercase text-[10px] px-6">Confirmar Pago</button>
                        </div>
                    </div>
                </div>
            )}

            <div className="bg-white-500 rounded-xl max-w-5xl w-full shadow-2xl border-2 border-t-8 border-red-600 flex flex-col max-h-[90vh]">

                {/* CABECERA */}
                <div className="p-6 border-b flex justify-between items-center bg-gray-50 text-black">
                    <div>
                        <h2 className="text-2xl font-black uppercase tracking-tighter italic leading-none text-red-600">Liquidación Final</h2>
                        <p className="text-[11px] font-bold text-gray-400 uppercase mt-1 tracking-widest italic">Expediente: #{viaje.numViaje}</p>
                    </div>
                    {!pagoCompletado && (
                        <button onClick={onClose} className="text-gray-400 hover:text-red-600 transition-colors"><FaTimes size={24} /></button>
                    )}
                </div>
                    ...
                    <div className="flex-1">
                        ...
                        <div className="space-y-3">
                            {viaje.vehiculos.map((v, i) => (
                                <div key={i} className="p-3 border rounded-lg bg-white-500 shadow-sm border-l-4 border-red-600">
                                    <div className="flex justify-between items-center">
                                        ...
                                        <div className="flex gap-4 text-center">
                                            {[
                                                ...
                                                <div key={idx} className="flex flex-col border-r last:border-none pr-4 last:pr-0">
                                                    <span className="text-[8px] font-black text-gray-400 uppercase">{c.label}</span>
                                                    <span className="text-[11px] font-black text-black">${parseFloat(c.val).toLocaleString()}</span>
                                                </div>
                                            ))}
                                        </div>
                                    </div>
                    ...
                    <div className="w-full md:w-80 bg-gray-50 p-6 rounded-xl border border-gray-200 text-black">
                        {pagoCompletado ? (
                            <div className="text-center animate-in zoom-in duration-300">
                                ...
                                <p className="text-[10px] font-bold text-gray-400 mt-1 uppercase italic italic tracking-tighter">Folio Contable: {folioGenerado}</p>

                                <ReactToPrint
                                    trigger={() => (
                                        <button ref={btnPrintRef} className="btn btn-error text-white-500 w-full mt-8 uppercase font-black tracking-widest gap-2">
                                            <FaPrint /> Imprimir Recibo
                                        </button>
                                    )}
                                    content={() => reciboRef.current}
                                />
                                <button onClick={onClose} className="btn btn-success h-14 w-full mt-4 text-white-500 font-black uppercase text-[12px] gap-2">
                            ...
                            <>
                                ...
                                <div className="space-y-2 border-t-2 border-dashed border-gray-300 pt-4">
                                    <h4 className="text-[10px] font-black text-gray-400 uppercase mb-4">Resumen Contable</h4>
                                    {[
                                        ...
                                        <div key={i} className="flex justify-between text-[11px] font-bold text-gray-600 uppercase italic">
                                            <span>{row.l}:</span>
                                            <span>${row.v.toLocaleString()}</span>
                                        </div>
                                    ))}

                                    <div className="flex justify-between text-2xl font-black text-gray-800 uppercase mt-6 pt-4 border-t-4 border-red-600 italic">
                                        <span>Total:</span>
                                        <span className="text-red-600 font-black">${granTotalReal.toLocaleString()}</span>
                                    </div>
                                </div>
