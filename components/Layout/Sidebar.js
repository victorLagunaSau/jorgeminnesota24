...
const Sidebar = ({onSelectModule, selectedModule}) => {
    ...
    const getButtonClass = (module) =>
        `w-full px-4 py-2 bg-white text-black text-[12px] font-bold text-left hover:bg-red-200 flex items-center ${
            selectedModule === module ? "bg-red-100 border-r-4 border-red-600" : ""
        }`;

    return (
        <aside className="fixed top-0 left-0 h-full w-64 bg-gray-800 shadow-lg mt-16 overflow-y-auto">
            <nav className="mt-10 mb-20">
                <ul className="w-56 mx-auto rounded-box text-black-500">
                    ...
                    <li>
                        <button
                            className="w-full px-4 py-2 bg-white text-black text-[14px] font-bold text-left hover:bg-red-200 flex items-center border-b border-gray-100"
                            onClick={() => toggleSubMenu('caja')}
                        >
                            <FaCashRegister className="mr-3 text-red-600"/> Caja
                        </button>
                        {openSubMenu.caja && (
                            <ul className="bg-gray-50 border-l-2 border-red-600">
